...
public class EventoService : IEventoService
{
	...
	public async Task<Dashboard_Response> Dashboard(DashboardRequest request)
	{
		...
		List<Models.Eventos.Dashboard_Item> aulas = new List<Models.Eventos.Dashboard_Item>() { };
			...
			foreach (TurmaList turma in turmas)
			{
				...
				else
				{
					...
					foreach (AlunoList aluno in alunosTurma)
					{
						...
						FeriadoResponse? feriado = feriados.FirstOrDefault(x => x.date.Date == data.Date);
						Models.Eventos.Dashboard_Item aula = new()
						{
	...
	public async Task<Dashboard> DashboardV2(DashboardRequest request)
	{
		...
		List<DashboardMesesRoteiro> meses = new List<DashboardMesesRoteiro>();
		...
		{
			var list = _mapper.Map<List<DashboardRoteiro>>(roteirosArray.Where(roteiro =>
			{
		...
		var aulasPorAluno = new Dictionary<int, List<DashboardAlunoAulaReposicao>>();
